...
public class EccSecretKey : AsymmetricSecretKey
{
    ...
    public static EccSecretKey Create(KeyMetadata metadata, ECDsa key) =>
        SecretKeyFactory.Create(key, bytes => new EccSecretKey(metadata, key.KeySize, bytes));
    ...
    /// <returns>The newly created <see cref="EccSecretKey"/> instance.</returns>
    public static EccSecretKey Create(KeyMetadata metadata, ECDiffieHellman key) =>
        SecretKeyFactory.Create(key, bytes => new EccSecretKey(metadata, key.KeySize, bytes));
    ...
    public EccSecretKey(KeyMetadata metadata, int curveSizeBits, ReadOnlySpan<byte> pkcs8PrivateKey, X509Certificate2? certificate = null)
        : base(metadata, curveSizeBits, pkcs8PrivateKey, certificate)
    {
