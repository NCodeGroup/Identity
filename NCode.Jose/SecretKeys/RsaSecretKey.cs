...
public class RsaSecretKey : AsymmetricSecretKey
{
    ...
    public static RsaSecretKey Create(KeyMetadata metadata, RSA key) =>
        SecretKeyFactory.Create(key, bytes => new RsaSecretKey(metadata, key.KeySize, bytes));
    ...
    public RsaSecretKey(KeyMetadata metadata, int keySizeBits, ReadOnlySpan<byte> pkcs8PrivateKey, X509Certificate2? certificate = null)
        : base(metadata, keySizeBits, pkcs8PrivateKey, certificate)
    {
